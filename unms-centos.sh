...
# Ubiquiti Network Management System (UNMS) install script for CentOS 8.
# Version 1.1
#
...
# Installing UNMS:
sudo curl -fsSL https://unms.com/v1/install > /tmp/unms_inst.sh && sudo yes | bash /tmp/unms_inst.sh

# Adding Docker netowrk interfaces to trusted zone in firewall:
sudo ip -o link show | awk -F': ' '{if ($2 ~/^br/) {print $2}}' >> brfaces.txt
sudo xargs -I {} -n 1 firewall-cmd --permanent --zone=trusted --change-interface={} < brfaces.txt
