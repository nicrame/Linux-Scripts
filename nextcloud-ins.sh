...
# Made for freshly installed, server Linux distributions using AMD64(x86_64)
# architecture: Debian (11,12), Enterprise Linux (9), Ubuntu Server (22),
# Fedora Server (39).
#
# It will update OS, preconfigure everything, install neeeded packages and
# Nextcloud. There is also support for upgrading Nextcloud and OS packages -
# just download and run latest version of this script again. It will create
# backup of current Nextcloud (but without users files) with it's database, and
# then it will upgrade OS, software packages, and Nextcloud to the newest major
# version.
#
# This Nextcloud installer allows Nextcloud to work locally and thru Internet:
# - by local IP address with and without SSL (it use self signed SSL
#   certificate for https protocol),
# - or using domain name (local and over Internet), if domain is already
#   configured correctly (it will use free Let's Encrypt service for certificate
#   signing).
# Software packages that are installed are Apache (web server), MariaDB
# (database server), PHP (programming language with interpreter), NTP (time
# synchronization service), and Redis (cache server) installed and used. Some
# other software is also installed for better preview/thumbnails generation by
# Nextcloud like LibreOffice, Krita, ImageMagick etc. Also new service for
# Nextcloud "cron" is generated that starts every 5 minutes so Nextcloud can do
# some work while users are not connected.
#
...
#
# You may also add specific variables (lang, mail, dns) that will be used, by
# adding them to command above, e.g:
...
# -lang (for language) variable will install additional packages specific for
#   choosed language and setup Nextcloud default language. Currently supported
#   languages are: none (default value is none/empty that will use web browser
#   language), Arabic (ar), Chinese (zh), French (fr), Hindi (hi), Polish (pl),
#   Spanish (es) and Ukrainian (uk),
# -mail variable is for information about Your email address, that will be
#   presented to let's encrypt, so you'll be informed if domain name SSL
#   certificate couldn't be refreshed (default value is empty),
# -dm variable is used when you got (already prepared and configured) domain
#   name, it will be configured for Nextcloud server and Let's encrypt SSL
#   (default value is empty),
# -nv variable allows You to choose older version to install, supported version
#   are: 24-28, empty (it will install newest, currently v28),
# -fdir variable gives possibility to specify where user files and
#   nextcloud.log files are stored, by default this settings will leave default
#   location that is /var/www/nextcloud/data. selecting different location will
#   not change Nextcloud configuration, but will bind (using mount) default
#   Nextcloud location, to the specified one, so using security mechanism like
#   chroot/jail/SELinux etc. will work correctly without additional
#   configuration for them, web server etc. For example if option
#   -fdir=/mnt/sdc5/nextcloud-data will be used, then entering directory
#   /var/www/nextcloud/data will actually show content of
#   /mnt/sdc5/nextcloud-data. If you want to use spaces between words in
#   directory name, then put path inside double quotes, eg.
#   -fdir="/mnt/sdx/users data folder" To remember data directory settings, and
#   mount them each OS start /etc/fstab file is modified.
# -restore argument is used for recovering older Nextcloud files/database.
#   Since v 1.11 this script generate backup of Nextcloud files (excluding users
#   data) and database, when it's started for upgrade process (which is default
#   scenario when script is started another time after first use). You may use
#   -restore=list to check the list of previously created backups, or
#   -restore=filename.tar.bz2 to select one of those files, and use them to
#   restore Nextcloud. IMPORTANT: When -restore argument is used with any kind
#   of parameters, then any other is ignored. It means You can't use -restore
#   variable with others.
# - backup argument starts backup process without doing any other tasks. It
#   will just create backup of current Nextcloud install with database,
#   excluding users files. Similar to -restore, -backup argument must be used
#   by itself (any other one used with it will be ignored).
#
# After install You may use Your web browser to access Nextcloud using local IP
# address, or domain name, if You have configured it before (DNS settings and
# router configuration should be done earlier by You). Both HTTP and HTTPS
# protocols are enabled by default. Localhost, self signed certificate is
# generated by default. For additional domain name certificate is made with
# Let's encrypt service (if You use -dns command variable).
#
# It was tested with many Nextcloud versions since v24.
#
# Updates of Nextcloud after using this script:
# By default this script disable "updatenotification" app that allow You to
# update Nextcloud using its own administration panel. The main reason is that
# such updates sometimes leave files that shouldn't stay, which brakes their
# update system at some points (i had many such problems in the past). So, to
# update Your Nextcloud there are two options:
#
# 1. You may start the script again, so it will upgrade OS with software
#   packages and Nextcloud to the newest version (it will update between major
#   releases too), so for example if You have version 28.0.3, it will update it
#   to 29.0.5(that was newest version when this text was edited). But if You
#   selected version to install with "-nv" argument (eg. -nv=28) when script was
#   used for the first time, then starting script again will not update
#   anything, and leave You with selected version, without updating minor
#   release. So if You got 28.0.3 it will not update to 28.0.9 (because when
#   this script is released, i do not know how many minor releases will be in
#   the future).
# 2. You may also enable updatenotification app using Nextcloud GUI - go to
#   Apps -> Disabled apps -> click on Enable button near "Update notification"
#   app. Then go to "Administration settings" -> Overview, where will be
#   information about new version available for updating.
#
# In case of problems, LOG output is generated at
# /var/log/nextcloud-installer.log. Attach it if You want to report errors with
# installation process.
#
# If You want to report errors that You think may be made by the script, please
# add logs of Apache web server, PHP and Nextcloud. This script was never
# tested, and not recommended to be used on containerization environment (like
# Docker, LXC etc.), but it was working well on virtual machines under KVM and
# Virtualbox.
#
# More info:
...
#
# License:
# 1. You use it at your own risk. Author is not responsible for any damage made
#   with that script.
# 2. Any changes of scripts must be shared with author with authorization to
#   implement them and share.
#
# V 1.11.6 - 19.07.2025
# - Small tweaks making the comments no more than 80 characters wide, fixing
#   typos, and changing spacing to two spaces.
# V 1.11.5 - 25.05.2025
...
# V 1.11.2 - 16.05.2024
# - new arguments: -backup (create backup) and -restore (that can be used with
#   "list" argument to show previously created backups, or with filename to be
#   used to restore from it)
# - modify backup file names to show more data (date, time and Nextcloud
#   version that is backed up)
# V 1.11 - 16.05.2024
...
# V 1.9.2 - 13.03.2024
# - checking if "fdir" parameter is configured for already existing directory
#   and inform if not
# - fix spaces in directory names saved in fstab, configured with -fdir
#   argument (fstab do not support spaces in directory names)
# V 1.9.1 - 12.03.2024
# - some description update, and few code changes that do not affect the way
#   script is working
# - add PHP 8.3 install code (currently disabled) for future NC versions
# V 1.9 - 04.03.2024
# - new argument that allow to configure location of "data" directory, where
#   user files are stored (it use mount/fstab for security mechanisms
#   compatibility)
# V 1.8.1 - 07.02.2024
# - first release with Fedora Server 39, and Ubuntu Server LTS (22)
#   distributions support
# V 1.8 - 04.02.2024
# - first release with Rocky Linux (9), and other Enterprise Linux
#   distributions support
...
# - packages installer will now wait for background jobs (started by OS) to
#   finish
# V 1.6.4 - 04.01.2024
...
# V 1.6.2 - 04.08.2023
# - few more languages are now supported with -lang= parameter (Arabic (ar),
#   Chinese (zh), French (fr), Hindi (hi), Polish (pl), Spanish (es) and
#   Ukrainian (uk))
# V 1.6.1 - 03.08.2023
# - small tweaks
# V 1.6 - 03.08.2023
# - new variable that allows installing older version of Nextcloud (users
#   reported problems with NC27)
# - the script rename itself after finished work (so installer command always
#   refer to newest version)
...
# V 1.5.3 - 15.04.2023
# - using older PHP (8.1) version for upgrade process before removing it
#   (Nextcloud do not finish upgrade process on never PHP version)
# - check for currently installed Nextcloud version and update it so many times
#   it needs (till version 26) - when upgrading from script version 1.4 or
#   older
# V 1.5.2 - 05.04.2023
# - twofactor_webauthn app installing and enabling for more security (tested
#   with Yubikey)
# V 1.5.1 - 05.04.2023
...
# - install miniupnpc ans start it for port 80 and 443 to open ports (it should
#   be unncessary)
# - added more variables to use (language, e_mail)
# - installer is now creating file with it's version number for future upgrades
# - installer detects if older versions of script were used, and in the next
#   release it will upgrade everything (nextcloud included)
# V 1.4.3 - 24.02.2023
# - allow self-signed certificate config option in nextcloud (it may be needed
#   sometimes)
# V 1.4.2 - 10.02.2023
# - completely disable opcache because of many segfaults even when JIT is
#   completely disabled
# V 1.4.1 - 08.02.2023
# - opcache jit cache in php has been disabled because of many segfaults
#   reported
# V 1.4 - 31.01.2023
# - fixes thanks to "maybe" user from hejto.pl portal (ufw, redis, chmods etc.)
#   Thank You!
# V 1.3 - 30.01.2023
...
# V 1.1 - 04.08.2022
# - added support for adding domain name as command line variable (with let's
#   encrypt support)
...
# - make backup of Nextcloud script (excluding users files) and database for
#   recovery before upgrade (done with v1.11)
...
# Check if Nextcloud was updated with nv variable, and if yes, skip doing
# anything to not brake it. This is version made for newer version of script,
# so it report that it was running under $ver_file.
...
function install_soft {
  ...
  if [ -e $elvf ]
  then
    # Disabling SELinux for testing purposes here. By default just
    # let it run and configure things instead.
...
then
  ...
  then
    ...
    then
      ...
      update_os
      nv_update
      # Installing additional packages added with v1.7 upgrade.
...
if [ "$lang" = "ar" ]
then
  echo "!!!!!!! Installing language package: Arabic" >> $insl 2>&1
  if [ -e $debvf ]
  then
...
if [ "$lang" = "zh" ]
then
  echo "!!!!!!! Installing language package: Chinese" >> $insl 2>&1
  if [ -e $debvf ]
  then
...
if [ "$lang" = "fr" ]
then
  echo "!!!!!!! Installing language package: French" >> $insl 2>&1
  if [ -e $debvf ]
  then
...
if [ "$lang" = "hi" ]
then
  echo "!!!!!!! Installing language package: Hindi" >> $insl 2>&1
  if [ -e $debvf ]
  then
...
if [ "$lang" = "pl" ]
then
  echo "!!!!!!! Installing language package: Polish" >> $insl 2>&1
  if [ -e $debvf ]
  then
...
if [ "$lang" = "es" ]
then
  echo "!!!!!!! Installing language package: Spanish" >> $insl 2>&1
  if [ -e $debvf ]
  then
...
if [ "$lang" = "uk" ]
then
  echo "!!!!!!! Installing language package: Ukrainian" >> $insl 2>&1
  if [ -e $debvf ]
  then
...
# Disabling info about creating free account on shared pages/links when logged
# out (because it is missleading for private nextcloud instances).
...
# Import certificate by Nextcloud so it will not cry that it can't check for
# mjs support by JavaScript MIME type on server. Actually it does not resolve
# the problem with information, so i think it is just another inside error
# ignored by NC.
...
# Disable .htaccess blocking because we use nginx that do not use it, also it
# should be handled by Nextcloud itself!
...
restart_websrv
# Additional things that may fix some unknown Nextcloud problems (that appeared
# for me when started using v19).
